'''
The following code is a MoE modified codes
'''
import torch
...
class MLP(nn.Module):
    '''
    The expert network for the MoE model
    '''
    def __init__(self, 
                 dropout:float, 
                 hidden_act:str, 
...
class SparseDispatcher(object):
    ...
    def __init__(self, num_experts, gates):
        ...
        # sort experts
        sorted_experts, index_sorted_experts = torch.nonzero(gates).sort(0)# sort the x and y by their index, index_sorted_experts is the position there it takes from
        # drop indices
...
class MoE(nn.Module):
    ...
    def __init__(self, 
        ...
        self.output_size = moe_output_size
        self.input_size = moe_input_size
        self.hidden_size = moe_hidden_size
        self.k = moe_k

        # instantiate experts
        self.experts = nn.ModuleList([MLP(self.dropout, self.hidden_act, self.input_size, self.output_size, self.hidden_size) for i in range(self.num_experts)])
    ...
    def noisy_top_k_gating(self, x, train, mask=None, noise_epsilon=1e-2):
        """Noisy top-k gating.
        ...
        if mask is not None:
            mask = mask.unsqueeze(-1).expand_as(clean_logits)
            clean_logits = clean_logits.masked_fill(mask == 0, float('-inf'))
        ...
        return gates, load #gate include 0 and prob; load only include the prob of each expert



    def forward(self, x, train=True, mask=None, loss_coef=1e-2):
        """Args:
        ...
        x = x.view(batch_size * length, hidden_dim)
        ...
        expert_inputs = dispatcher.dispatch(x)#a list, each element is the input for that expert
        gates = dispatcher.expert_to_gates()
        expert_outputs = [self.experts[i](expert_inputs[i]) for i in range(self.num_experts)]
        y = dispatcher.combine(expert_outputs)
        y = y.view(batch_size, length, hidden_dim)
        return y, loss
...
if __name__ == "__main__":
    ...
    mask = torch.randint(0, 2, (10, 100)).bool()  # Example mask with shape [batch, length]
    y, loss = model(x, mask=mask)
    import pdb; pdb.set_trace()
    print(y, loss)
