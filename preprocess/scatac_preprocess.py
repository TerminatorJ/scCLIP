# This file contains some utility functions that are adapted from the GET paper
# We aim to process the single-cell ATAC-seq dataset and store it for model training
...
def query_motif(peak_bed, motif_bed, outpath):
    """
    ...
    subprocess.run(
        ...
        stdout=open(f"{outpath}/query_motif.bed", "w"),
        check=True,
    )
    return "query_motif.bed"


def get_motif(peak_file, motif_file, outpath):
    """
    ...
    Returns:
        str: Path to the output bed file containing the peak motif data.
    """
    OUTPUT_PATH = f"{outpath}/get_motif.bed"
    cmd = f"""
    ASSEMBLY="hg38"
    MEM="12G"
    CPU="2"
...
def create_peak_motif(peak_motif_bed, output_zarr, peak_bed):
    ...
    # Pivot the data, set the columns as the motif models
    ...
    z.create_dataset("bulk_peak_names", data=name_values)
    z.create_dataset("bulk_motif_names", data=motif_values)
...
def add_exp(
    ...
    """
    # Initialize Gencode
    gencode = Gencode(assembly=assembly, version=version)#The genecode include the promotor information
    ...
    exp = (
        ...
        .mean()
        .reset_index()
    ) #if the peak overlaps with two genes, it will assign a mean expression value
